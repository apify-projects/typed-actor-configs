...
export type ArrayItems = FromSchema<
    typeof arrayItems,
    {
...
type inferArrayObjectProperty<T extends Exclude<(ArrayItems & { type: 'object' })['properties'], undefined>[string]> =
    T extends SubSchemaStringEnumProperty
...
type inferArrayObjectItems<T extends ArrayItems & { type: 'object' }> = T extends {
    properties: Record<string, { default?: unknown } | {}>;
}
    ? Resolve<
          {
              -readonly [Key in keyof T['properties'] &
                  (RequiredKeys<T> | DefaultedFields<T>)]: inferArrayObjectProperty<T['properties'][Key]>;
          } & {
              -readonly [Key in Exclude<
                  keyof T['properties'],
                  RequiredKeys<T> | DefaultedFields<T>
              >]?: inferArrayObjectProperty<T['properties'][Key]>;
          }
      >
    : unknown;
