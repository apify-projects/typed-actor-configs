...
import { DefaultedFields, Resolve, TrimTitleAndDescription } from './json-schemas/types.ts';
type InputSchema = {
    title: string;
    schemaVersion: 1;
    description?: string;
...
export type InferInput<T extends MinimalInputSchema> = Resolve<{
    -readonly [Key in keyof T['properties'] & (requiredKeys<T> | DefaultedFields<T>)]: inferProperty<T['properties'][Key]>;
} & {
    -readonly [Key in Exclude<keyof T['properties'], requiredKeys<T> | DefaultedFields<T>>]?: inferProperty<T['properties'][Key]>;
}>;
type inferProperty<T extends MinimalInputSchema['properties'][string]> = T extends StringEnumProperty ? InferEnumProperty<T> : T extends IntegerProperty ? InferIntegerProperty<T> : T extends NumberProperty ? InferNumberProperty<T> : T extends BooleanProperty ? InferBooleanProperty<T> : T extends ResourceProperty ? InferResourceProperty<T> : T extends ResourceArrayProperty ? InferResourceArrayProperty<T> : T extends ArrayProperty ? InferArrayProperty<T> : T extends StringProperty ? InferStringProperty<T> : T extends ObjectProperty ? InferObjectProperty<T> : never;
export declare function defineActorInput<InputConfiguration extends InputSchema>(path: `${string}/input_schema.json`, inputSchema: InputConfiguration): InputConfiguration;
export {};
